...
$.ajaxSetup({
    statusCode: {
        400: function () {
            location.href = "/Authorize/Index?returnUrl=" + encodeURIComponent(window.location.href);
        }
    }
});
...
//            $(this).removeClass("feedback")
        ...
        var e = function () {
            var h = $(this), b = h.attr("href"); if (!b) { setTimeout(function () {
               ...
               // h.removeClass("feedback") 
              }; var tk = gettk(), ak = argsArr["ak"], tgid = argsArr["tgid"], ism = argsArr["ism"], sc = argsArr["sc"]; if (!tk) tk = $("#TkrTkEncode").val(); if (!ak) ak = $("#TkrAk").val(); if (!tgid) tgid = $("#tgid").val(); if (!ism) ism = $("#ism").val(); if (!sc) sc = 640; var p = "tk=" + tk + "&ak=" + ak + "&tgid=" + tgid + "&ism=" + ism + "&sc=" + sc; b = b + (b.indexOf("?") > 0 ? "&" : "?") + p; if (b != undefined && b != "" && b.indexOf("#") == -1) { OpenUrl((b.substring(0, 1) == "/" || b.indexOf("http://") > -1 ? "" : "/") + b) }
        };
...
function SetCookie(sName, sValue, timeKeep) { var now = new Date(); var expireTime = new Date(now.valueOf() + timeKeep * 60000 * 60); document.cookie = sName + "=" + escape(sValue) + "; path=/; expires=" + expireTime.toGMTString() + ";"; }
function GetCookie(sName) { var aCookie = document.cookie.split("; "); for (var i = 0; i < aCookie.length; i++) { var aCrumb = aCookie[i].split("="); if (sName == aCrumb[0]) { return unescape(decodeURIComponent(aCrumb[1])); } } return ""; }
...
function openWin(url) {
    ...
    window.open(url, name, 'height=400,width=450,top=' + iTop + ',left=' + iLeft + ',status=no,toolbar=no,menubar=no,location=no,resizable=no,scrollbars=0,titlebar=no');
}
function browser() {
